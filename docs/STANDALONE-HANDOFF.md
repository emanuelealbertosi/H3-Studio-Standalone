...
|---|---|---|---|
| H3 Studio originale | `F:\H3-Studio` | Prodotto corrente basato su ComfyUI esterna | `3bf5731`, repository GitHub originale |
...
standalone. Il riferimento sorgente corrente è `3bf5731`. **Non importare ancora
...
- persistenza Admin del modello Anima/Nova AM resa resistente al salvataggio immediato, con conferma del modello restituito dal server e regressione su riapertura dello store (`3bf5731`).
- `Mantieni proporzioni` esteso da I2V a Keyframes, Reference, Continue ed Edit: Picture 1 guida I2V/Keyframes, Video 1 guida Continue/Edit e il workflow usa il primo visual disponibile in Reference (`3bf5731`).
- Libreria immagini allineata alla sorgente di Assets: Personaggi, Oggetti, Luoghi e immagini senza tag mostrano anche i job moderni, con rinomina, eliminazione multipla e invio allo Studio; etichetta Paesaggio rinominata Luogo mantenendo il valore interno `background` (`3bf5731`).
- Terminologia pubblica resa indipendente dal modello: UI, errori e documentazione usano LLM/LLM Vision; catalogo Chat e nodo locale accettano qualunque GGUF non-projector, conservando i vecchi identificatori tecnici solo per compatibilità (`3bf5731`).
- Libreria: eliminazione singola o multipla dei montaggi con conferma, cascata atomica dei soli collegamenti clip e conservazione di video sorgente, varianti e media (`3bf5731`).
...
ComfyUI è avanzata fino a 3bf5731: leggi la coda di integrazione nella sezione 11
